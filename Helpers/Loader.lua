...
function Loader:load(FilePath,Name,FontSize,SpecialRequest,Prefix)
	...
	local exceptions = {"db","txt","mod","gui","tmx"} ---to skip files, (used to be to be to skip json files)
	...
	if(e == mapType)then Type = "Map" end
	...
	if(Type ~= "NoGo")then --for skipping files
		...
		if(Type ~= "Font" and Type ~= "Sound" and Type ~= "Map")then 
			stuffToLoad[#stuffToLoad+1] = ob;
			totalNum = totalNum + 1;
		elseif(Type == "Map")then
		  ob.map = STI.new(ob.filePath,{"robotics"})
		  
		  totalMaps = totalMaps + 1;
		  mapsToLoad[#mapsToLoad+1] = ob;
		  mapsHolder[ob.name] = ob.map;
